...
@section("content")
                                           ...
                                           <td class="libelle">{{$role->libelle}}</td>
                                            ...
                                            <td>
                                                <div class="btn-group" role="group" aria-label="Basic outlined example">
                                                    <button class="edit_role" value="{{$role->id}}" style="border: none" type="button" class="btn btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#depedit"><i class="icofont-edit text-success"></i></button>
                                                    <button type="button"  attr_id ="{{ $role->id }}"class ="Bdelete_btn" style="border: none" class="sup_role" class="btn btn-outline-secondary deleterow"><i class="icofont-ui-delete text-danger"></i></button>
                                                </div>
                                            </td>
                                        </tr>
                                        @empty
        ...
        <div class="modal fade" id="depadd" tabindex="-1"  aria-hidden="true">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title  fw-bold" id="depaddLabel">Ajouter role</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form method="post" action="{{ route('parametre.addrole') }}">
                    @csrf
        ...
        <div class="modal fade" id="depedit" tabindex="-1"  aria-hidden="true">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title  fw-bold" id="depeditLabel">Modifier role</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('parametre.updaterole') }}" class="forms-sample" method="post">
                    @csrf
                <div class="modal-body">
    ...
    $("#myProjectTable").on("click",".edit_role",function(){
        ...
        $(document).on('click','.Bdelete_btn',function (e){
            ...
            if(confirm("do you want to delete this file?")) {
                $.ajax({
                    type: 'post',
                    url: "{{route('parametre.deleterole')}}",
                    data: {
                        '_token': "{{csrf_token()}}",
                        'id': offer_id,
                    },
